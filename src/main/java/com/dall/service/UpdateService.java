...
public class UpdateService {
    ...
    /**
     * Get all the scraped ad from the sheets and update them.
     */
    public void updateSpreadSheet() {
        ...
        log.info("Updated {} ad{} for city centre.",
            serializedAdsFromCityCentre.size(),
            serializedAdsFromCityCentre.size() > 1 ? "s" : "");
        writeService.rawWriteToSheet(serializedAdsFromSuburb, getRangeForSuburb());
        log.info("Updated {} ad{} for suburb.",
            serializedAdsFromSuburb.size(),
            serializedAdsFromSuburb.size() > 1 ? "s" : "");
    }
    ...
    private List<Ad> parseRawLines(List<List<Object>> rawLines) {
        return rawLines.stream()
            ...
            .map(rawLine -> new Ad(rawLine.get(0).toString(),
    ...
    private List<List<Object>> serializeAds(List<Ad> ads) {
        ...
        for (Ad ad : ads) {
            serializedAds.add(Arrays.asList(ad.getShortenedLink(),
                ad.getAddress(),
                ad.getDistrict(),
                ad.getLeaseTime(),
                ad.getBathrooms(),
